...
需要强调的是，动态语言也是有类型的：每个对象，不管是基础的还是用户自定义的，都有类型。多数动态语言没有类型声明，意味着我们不能告诉编译器值的类型，所以通常类型无从说起。而使用静态语言时，我们必须告诉编译器值的类型，因为类型仅存在于编译时，在运行时则不能更改。在 Julia 中，类型本身就是运行时对象，同时它也可以把信息传递给编译器。
