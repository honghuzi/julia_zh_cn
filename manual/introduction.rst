...
由于 Julia 的编译器不同于其它语言比如 Python 或 R 所使用的解释器，一开始您或许不能直观的体会到Julia的效率之高。如果您发现 Julia 变慢了，我们非常建议您在尝试其它功能前读一下 :ref:`man-performance-tips` 。只要您理解 Julia 的工作方式，就会很容易地写出运行效率甚至可以和 C 相媲美的代码。
...
它提供了简易和富有表现力的高等数值计算，它类似于 R 、 MATLAB 和 Python ，支持一般用途的编程。
